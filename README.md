...
Some of this code is originally from Michael Kohn, the creator of naken_asm, and has only been documented with minor changes.
...
You will need the downloaded naken_asm, and configure it in the makefile, and run the compilation command ```make PROJECT_FOLDER=folder``` if you want to compile the "1-triangle" example, then you should write: ```make PROJECT_FOLDER=1-triangle```